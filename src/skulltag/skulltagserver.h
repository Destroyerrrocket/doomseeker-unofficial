...
class SkulltagServer : public Server
{
	...
	public:
		...
		{
			GAMEMODE_COOPERATIVE 		= GameMode::SGMICooperative,
			GAMEMODE_SURVIVAL 			= 0,
			GAMEMODE_INVASION 			= 1,
			GAMEMODE_DEATHMATCH  		= GameMode::SGMIDeathmatch,
			GAMEMODE_TEAMPLAY 			= GameMode::SGMITeamDeathmatch,
			GAMEMODE_DUEL 				= 2,
			GAMEMODE_TERMINATOR 		= 3,
			GAMEMODE_LASTMANSTANDING 	= 4,
			GAMEMODE_TEAMLMS 			= 5,
			GAMEMODE_POSSESSION 		= 6,
			GAMEMODE_TEAMPOSSESSION 	= 7,
			GAMEMODE_TEAMGAME 			= 8,
			GAMEMODE_CTF 				= GameMode::SGMICTF,
			GAMEMODE_ONEFLAGCTF 		= 10,
			GAMEMODE_SKULLTAG 			= 11,
			GAMEMODE_DOMINATION 		= 12
		};
